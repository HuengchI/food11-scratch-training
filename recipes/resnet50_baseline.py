...
def load_img(fp):
    return np.array(Image.open(fp).convert("RGB").resize((img_size, img_size)))
...
def gpu_mixed_strategy(imgs, labels, epoch):
    if epoch < 275:
        mix_prob = 0.80
    else:
        mix_prob = 0.0
...
@torch.no_grad()
def strict_bn_calibration(ema_model, pure_tensor, mean, std, batch_size):
    ema_model.train()
    ...
    for i in range(0, len(pure_tensor), batch_size):
        imgs = pure_tensor[i:i+batch_size].float() / 255.0
        imgs = (imgs - mean) / std
...
def main():
    ...
    for epoch in range(n_epochs):
        ...
        for s in range(steps_per_epoch):
            ...
            imgs, l_a, l_b, lam = gpu_mixed_strategy(imgs, labels, epoch)
        ...
        if epoch >= 290 and epoch % 2 == 0:
            strict_bn_calibration(
                ema, train_manifold.data, mean, std, batch_size)
        ...
        if epoch_val_ema_acc > best_ema_acc:
            ...
            torch.save(ema.state_dict(), "best_food11_resnet50_90plus.pt")
